const displayNotes = (() => {
        ...
        html += `
            <div class="card mx-4 my-2 bg-dark text-white thatsMyNote" style="width: 18rem;">
                <div class="card-body">
                    ...
                    <button id="${index}" onclick=deleteNotes(this.id) class="DelBtn">Usun</button>
                </div>
            </div>
        `
    });
    ...
    if (notesObj.length != 0) {
        ...
        noteEle.innerHTML = '<h3 style="text-align: center; color: grey;">Brak do wyswietlenia</h3>'
    }
})
    ...
    Array.from(cards).forEach((ele) => {
        ...
        if (cardText.includes(inputText)) {
            ...
            if (countNone === cards.length) {
                document.getElementsById('noMatches').innerHTML = `<h3 style="text-align: center; color: grey;">brak</h3>`
            } else {
